...
const TEAM_EMAILS = [
  "jcgomes@salesgroup.pt", "jcgomes67@gmail.com", "cmendes@salesgroup.pt", 
  "cchau@salesgroup.pt", "vsilva@salesgrouop.pt", "pbacalhau@salesgroup.pt"
];
...
export default function App() {
  ...
  const handleDeleteTicket = async (id) => {
    if (window.confirm("Deseja eliminar definitivamente esta tarefa?")) {
  ...
  const exportToExcel = () => {
    const dataToExport = filteredTickets.map(t => ({
      ...
      Setor: t.type, Executor: t.assignedTo, Solicitante: t.userEmail, Criado: t.createdOn, Entrega: t.endDate
    }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans text-gray-900">
      ...
      <div className="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <button onClick={() => setShowHidden(!showHidden)} className="text-xs font-bold text-blue-600 flex items-center gap-1 mt-1 hover:underline">
              {showHidden ? <Eye size={14}/> : <EyeOff size={14}/>} {showHidden ? "Ocultar Arquivados" : "Ver Arquivados"}
            </button>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={exportToExcel} className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all"><Download size={18} /> Excel</button>
          <button onClick={() => { setEditingTicket(null); setIsModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all"><PlusCircle size={20} /> Nova</button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto mb-6 relative">
        ...
        <input type="text" placeholder="Pesquisa rápida..." className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50/50 border-b">
              <tr>
                ...
                <th className="p-4">
                  <div onClick={() => requestSort('priority')} className="text-[10px] font-bold text-gray-400 uppercase cursor-pointer flex items-center gap-1 mb-2 hover:text-blue-600">Prio <ArrowUpDown size={12}/></div>
                ...
                <th className="p-4">
                  ...
                  <input type="text" placeholder="Filtrar..." className="w-full p-1 text-xs border rounded outline-none focus:border-blue-400" onChange={e => setColumnFilters({...columnFilters, assignedTo: e.target.value})} />
                </th>
                ...
                <React.Fragment key={ticket.id}>
                  <tr className={`transition-colors ${ticket.hidden ? 'opacity-50 grayscale bg-gray-50' : ''} ${expandedRow === ticket.id ? 'bg-blue-50/30' : 'hover:bg-gray-50/50'}`}>
                    <td className="p-4">
                      ...
                      <div className="text-[10px] text-gray-400 uppercase">{ticket.createdOn} | {ticket.type}</div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right space-x-1 whitespace-nowrap">
                      <button onClick={() => toggleHideTicket(ticket.id, ticket.hidden)} title="Arquivar/Restaurar" className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 transition-colors">
                      ...
                      <td colSpan="5" className="p-6 text-sm">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          ...
                          <div className="space-y-4">
                            ...
                            <p className="flex items-center gap-2 text-gray-600 text-xs px-2"><Clock size={14}/> <b>Previsão:</b> {ticket.endDate || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
                    </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-8 text-left animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">{editingTicket ? 'Editar Tarefa' : 'Nova Tarefa'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full"><X className="text-gray-400" /></button>
            </div>
            <form onSubmit={handleSaveTicket} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Assunto da Tarefa</label>
                <input name="subject" defaultValue={editingTicket?.subject} required className="w-full bg-gray-50 border rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: Ajuste de faturas..." />
              </div>
              
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Descrição</label>
                <textarea name="description" defaultValue={editingTicket?.description} rows="3" className="w-full bg-gray-50 border rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Detalhes técnicos..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Solicitante</label>
                  <select name="userEmail" defaultValue={editingTicket?.userEmail || "jcgomes@salesgroup.pt"} className="w-full bg-gray-50 border rounded-xl p-3 text-sm outline-none">
                ...
                <div className="space-y-1">
                  ...
                  <select name="assignedTo" defaultValue={editingTicket?.assignedTo} required className="w-full bg-blue-50 border-blue-200 border rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold text-blue-800">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select name="type" defaultValue={editingTicket?.type} className="w-full bg-gray-50 border rounded-xl p-3 text-sm outline-none">
                ...
                <div className="space-y-1">
                  ...
                  <select name="priority" defaultValue={editingTicket?.priority} className="w-full bg-gray-50 border rounded-xl p-3 text-sm outline-none">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Data de Entrega</label>
